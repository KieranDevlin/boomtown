...
function generateToken(user, secret) {
  ...
  const token = jwt.sign({ id, email, fullname, bio }, secret, {
    expiresIn: '2h'
  }); //expiresIn option object can be strings or seconds as INT
  return token;
}
...
const mutationResolvers = app => ({
  async signup(
    ...
    try {
      ...
      const user = await pgResource.createUser({
        //if key and value are the same, you can use just the name in the object
        fullname,
        email,
  ...
  //TODO TRY CLEARING COOKIES IF CANT LOGOUT CATCH RETURN FALSE
